...
As in the running nlmixr2 vignette, let's start with a very simple PK
...
nlmixr2 provides the ability to:
...
piping the fit or the nlmixr2 model specification function to a `model`
...
then re-estimate by piping it to the `nlmixr2` function again.
...
```{r}
# Note currently cov is needed as a prefix so nlmixr2 knows this is an
# estimated parameter not a parameter
wt70 <- fit %>% # FIXME could be based on if it finds the covariate in the last used nlmixr2 data.

...
are modeled in nlmixr2 using the `~` "modeled by" specification.

To see more about how nlmixr2 will handle the multiple compartment
...
(using the `cmt` and `dvid` data items) to allow nlmixr2 to model the
...
part of the `nlmixr2` model. This is because `cp` is not a defined
...
the observations.  `nlmixr2` expects the `cmt` data item to match the
...
nlmixr2, you can set the following option:
...
v1s <-
  nlmixr2::vpc(fit.TOS, show=list(obs_dv=T), scales="free_y") +
  ylab("Warfarin Cp [mg/L] or PCA") +
  xlab("Time [h]");
v2s <-
  nlmixr2::vpc(fit.TOS, show=list(obs_dv=T), pred_corr = TRUE) +
...
v1f <-
  nlmixr2::vpc(fit.TOF, show=list(obs_dv=T), scales="free_y") +
  ylab("Warfarin Cp [mg/L] or PCA") +
  xlab("Time [h]");
v2f <-
  nlmixr2::vpc(fit.TOF, show=list(obs_dv=T), pred_corr = TRUE) +
  ylab("Prediction Corrected Warfarin Cp [mg/L] or PCA") +
  xlab("Time [h]")

...
## To allow nlmixr2 to reload runs without large run times
## To run the actual models on your system, take the save options off.
options(nlmixr2.save=TRUE,
        nlmixr2.save.dir=system.file(package="nlmixr.examples"));
...
model outputs into data frames.  nlmixr2 supports the `tidy` and
...
Note in nlmixr2 it is possible to have more than one fit metric (based
...
in nlmixr2, not the back-transformed values in the table from the
...
exponentiated scale.  This is the reason why the default for nlmixr2's
...
used in nlmixr2 tables (ie `fit$parFixedDf`).  **Please note that this
...
The default R method `confint` works with nlmixr2 fit objects:
...
tidy dataset.  The easiest way to get these values in a nlmixr2 dataset
is to use:
...
In this case we can extract this information from a nlmixr2 fit object by:
...
You may notice warnings and `(NA)` in the table.  `nlmixr2` allows you
